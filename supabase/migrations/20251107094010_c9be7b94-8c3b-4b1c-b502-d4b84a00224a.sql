...
DROP POLICY IF EXISTS "auth can view package docs for open/approved tenders" ON public.tender_package_documents;
DROP POLICY IF EXISTS "auth can view line items for open/approved tenders" ON public.tender_line_items;
DROP POLICY IF EXISTS "bidder or issuer can view bids" ON public.tender_bids;
DROP POLICY IF EXISTS "builders can submit bids to open tenders" ON public.tender_bids;
DROP POLICY IF EXISTS "bidder can update own bid" ON public.tender_bids;
DROP POLICY IF EXISTS "view bid line items (bidder or issuer)" ON public.tender_bid_line_items;
DROP POLICY IF EXISTS "insert bid line items (bidder only)" ON public.tender_bid_line_items;
DROP POLICY IF EXISTS "update bid line items (bidder only)" ON public.tender_bid_line_items;
