...
export const useDocuments = (projectId?: string) => {
  ...
  // ✅ COMPREHENSIVE MIME TYPE MAPPING - FIXED
  ...
  // ✅ CLEAN FILE EXTENSION EXTRACTION - FIXED
  ...
  // ✅ RESOLVE CORRECT MIME TYPE - FIXED
  const resolveMimeType = (file: File): string => {
    ...
    return extensionToMime[fileExt] || 'application/octet-stream';
  };

  // ✅ GET CLEAN FILE TYPE DISPLAY NAME - FIXED
  const getFileTypeDisplayName = (document: Document): string => {
