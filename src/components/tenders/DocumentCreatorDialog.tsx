...
const getTemplateWithData = (docName: string, projectData: any, tenderData: any): DocumentSection[] => {
  ...
  const templates: Record<string, DocumentSection[]> = {
    ...
    'General Conditions of Contract': [
      { id: '1', title: 'Definitions and Interpretation', content: `In this Contract:\n"Employer" means ${tenderData?.client_name || '[Employer Name]'}\n"Contractor" means [Contractor Name]\n"Project" means ${tenderData?.title || '[Project Name]'}\n"Site" means ${tenderData?.project_address || '[Site Address]'}\n"Contract Price" means ${tenderData?.budget ? '$' + Number(tenderData.budget).toLocaleString() : '[Contract Sum]'}\n"Completion Date" means ${tenderData?.estimated_start_date && tenderData?.completion_weeks ? formatDate(new Date(new Date(tenderData.estimated_start_date).getTime() + tenderData.completion_weeks * 7 * 24 * 60 * 60 * 1000).toISOString()) : '[Completion Date]'}` },
    ...
    'Contract Schedules': [
      { id: '1', title: 'Schedule of Work Stages', content: `Stage 1: Site Establishment and Preliminaries\nDuration: [2] weeks\nCompletion: [Date]\n\nStage 2: Substructure and Foundations\nDuration: [4] weeks\nCompletion: [Date]\n\nStage 3: Superstructure and Frame\nDuration: [8] weeks\nCompletion: [Date]\n\nStage 4: External Envelope\nDuration: [6] weeks\nCompletion: [Date]\n\nStage 5: Internal Fit-out and Services\nDuration: [${tenderData?.completion_weeks ? Math.floor(tenderData.completion_weeks * 0.4) : 'X'}] weeks\nCompletion: [Date]\n\nStage 6: Finishes and Completion\nDuration: [4] weeks\nCompletion: ${formatDate(tenderData?.estimated_start_date && tenderData?.completion_weeks ? new Date(new Date(tenderData.estimated_start_date).getTime() + tenderData.completion_weeks * 7 * 24 * 60 * 60 * 1000).toISOString() : undefined)}` },
      { id: '2', title: 'Payment Schedule and Milestones', content: `Total Contract Value: ${tenderData?.budget ? '$' + Number(tenderData.budget).toLocaleString() : '[Amount]'}\n\nPayment Milestones:\n\nDeposit (10%): ${tenderData?.budget ? '$' + (Number(tenderData.budget) * 0.1).toLocaleString() : '[Amount]'}\nDue: Upon signing contract\n\nCompletion of Foundations (15%): ${tenderData?.budget ? '$' + (Number(tenderData.budget) * 0.15).toLocaleString() : '[Amount]'}\nDue: Upon completion and inspection\n\nFrame Stage (20%): ${tenderData?.budget ? '$' + (Number(tenderData.budget) * 0.2).toLocaleString() : '[Amount]'}\nDue: Upon lockup stage\n\nServices Rough-in (15%): ${tenderData?.budget ? '$' + (Number(tenderData.budget) * 0.15).toLocaleString() : '[Amount]'}\nDue: Upon completion of services installation\n\nFit-out Stage (20%): ${tenderData?.budget ? '$' + (Number(tenderData.budget) * 0.2).toLocaleString() : '[Amount]'}\nDue: Upon substantial internal completion\n\nPractical Completion (15%): ${tenderData?.budget ? '$' + (Number(tenderData.budget) * 0.15).toLocaleString() : '[Amount]'}\nDue: Upon issue of Practical Completion Certificate\n\nFinal Payment (5%): ${tenderData?.budget ? '$' + (Number(tenderData.budget) * 0.05).toLocaleString() : '[Amount]'}\nDue: End of defects liability period\n\nRetention: 5% held until defects liability period expires` },
