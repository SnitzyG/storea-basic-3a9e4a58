...
export const CreateTenderWizard = ({
  ...
  const [specFiles, setSpecFiles] = useState<Array<{
    id: string;
    name: string;
    path: string;
    type: string;
  }>>([]);
  const [sowFiles, setSowFiles] = useState<Array<{
    id: string;
    name: string;
    path: string;
    type: string;
        ...
        <div className="space-y-6">
          ...
          {step === 1 && <div className="space-y-4">
              <div>
                ...
                <Input id="title" placeholder="Enter tender title..." value={title} onChange={e => setTitle(e.target.value)} className="mt-1" />
              </div>

              <div>
                <Label htmlFor="message">Message *</Label>
                <Textarea id="message" placeholder="Describe the tender scope and requirements..." value={message} onChange={e => setMessage(e.target.value)} className="min-h-[120px] mt-1" />
              </div>
          ...
          {step === 2 && <div className="space-y-4">
              <Card>
                ...
                <CardContent>
                  <FileSelector projectId={projectId} selectedFiles={specFiles} onFileSelect={file => setSpecFiles([file])} onFileRemove={fileId => setSpecFiles([])} onUploadNew={files => {
                if (files.length > 0) {
                  const newFile = {
              ...
              <Card>
                ...
                <CardContent>
                  <FileSelector projectId={projectId} selectedFiles={sowFiles} onFileSelect={file => setSowFiles([file])} onFileRemove={fileId => setSowFiles([])} onUploadNew={files => {
                if (files.length > 0) {
                  const newFile = {
                    id: Math.random().toString(),
                    name: files[0].name,
          ...
          <div className="flex justify-between pt-4">
            <div className="flex gap-2">
              {step > 1 && <Button type="button" variant="outline" onClick={() => setStep(step - 1)} disabled={loading}>
                  Previous
                </Button>}
            ...
            <div className="flex gap-2">
              ...
              {step < totalSteps ? <Button type="button" onClick={() => setStep(step + 1)} disabled={loading || step === 1 && (!title.trim() || !message.trim())}>
                  Next
